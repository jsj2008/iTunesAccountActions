...
class iTunes extends spCurl
{
	...
	//protected $curlopt_useragent = 'iTunes/10.6.3 (Windows; Microsoft Windows 7 x64 Business Edition Service Pack 1 (Build 7601)) AppleWebKit/534.57.2';
	private $errormsg;
	...
	public function login()
	{
		...
		if (!preg_match('#<key>passwordToken</key><string>([a-zA-Z0-9\+\=\/]+)</string>#', $ret, $m)) {
			...
			#Get error message
			$m = array();
			if (preg_match('#<key>message</key><string>([a-zA-Z0-9\+\=\/]+)</string>#', $ret, $m))
				$this->errormsg = $m[1];
			return(false);
		}
